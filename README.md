...
- 底部：**软重启 system_server + SystemUI** 主按钮 + **刷新读数** 次按钮 + 构建版本/日期
...
- 滑块恢复顺序：`persist.hyperbrightness.sunlight.target`（hook 真正读取的源）→ App prefs →
  当前生效值 → 出厂下限；越界夹紧到边界。重装 App、清数据都不会再「忘记」设置
- 只在值真正变化时回写：轻点滑块不再把 prop 覆盖成当时的滑块位置（旧版会因此把目标
  重置成出厂 800 nit）
- 读数分来源显示：`已保存目标` / `生效状态` / `出厂上限（参考）` / `system_server 生效值`，
  出厂参考值不会再被误认为当前上限
